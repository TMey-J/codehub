...
import { FolderPlus, Trash2, Edit3, Upload, Globe, Lock, Code, File, ArrowLeft ,ChevronLeft, ChevronRight,Folder} from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchUserRepos = async () => {
        try {
            setIsLoading(true);
            // درخواست ۹ آیتم برای صفحه فعلی
            const { data } = await repoService.getUser(currentPage, 9);
            if (data.is_success) {
    ...
    const fetchRepoFolders = async (repoId) => {
        ...
        try {
            // دریافت کل فایل‌های پروژه از بک‌اند
            const { data } = await fileService.getFiles(repoId);
            if (data.is_success) {
                const files = data.response || [];
                // استفاده از Set برای جلوگیری از مسیرهای تکراری (Root همیشه هست)
                const folders = new Set(['/']); 
                
                files.forEach(file => {
                    const parts = file.relative_path.split('/');
                    // اگر فایل داخل یک یا چند پوشه باشد
                    if (parts.length > 1) {
                        let currentPath = '';
                        // ساختار درختی مسیرها را تا قبل از نام فایل استخراج می‌کنیم
                ...
                // تبدیل Set به آرایه و مرتب‌سازی الفبایی
    ...
    try {
        ...
        if (data.is_success) {
            ...
            // این کد را اضافه کنید تا مقدار اینپوت پاک شود
    ...
    return (
        <div className="ch-repo-container content-fade-in">
            <header className="repo-inner-header">
                <div className="repo-main-info" style={{ alignItems: 'center',textAlign:'left' ,direction:'ltr'}}>
                    <div>
                    ...
                    <form onSubmit={handleUploadSubmit}>
                        {uploadMode === 'files' && (
                            <div className="field" style={{ marginBottom: '20px' }}>
                                <label style={{ position: 'relative', top: '0', transform: 'none', color: 'var(--ch-text-dim)', display: 'block', marginBottom: '10px', fontSize: '0.9rem' }}>
                                    مسیر آپلود را انتخاب کنید (Root پیش‌فرض است)
                                </label>
                                
                                {isFetchingFolders ? (
                                    <div style={{ textAlign: 'center', padding: '20px', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', border: '1px solid var(--ch-border)' }}>
                                        <div className="luminous-loader" style={{ width: '24px', height: '24px', borderWidth: '2px', margin: '0 auto 10px' }}></div>
                                        <span style={{ color: 'var(--ch-text-dim)', fontSize: '0.85rem' }}>در حال بارگذاری ساختار پوشه‌ها...</span>
                                    </div>
                                ) : (
                                    <div style={{
                                        maxHeight: '160px',
                                        ...
                                        {repoFolders.map((folder, idx) => {
                                            const isSelected = targetPath === (folder === '/' ? '' : folder);
                                            
                                            // محاسبه هوشمند عمق پوشه و تفکیک نام اصلی از مسیر برای نمایش درختی
                                            const isRoot = folder === '/';
                                            const parts = folder.split('/');
                                            const depth = isRoot ? 0 : parts.length;
                                            const displayName = isRoot ? 'پوشه اصلی (Root)' : parts[parts.length - 1];
                                            
                                            return (
                                                <div
                                                    key={idx}
                                                    onClick={() => setTargetPath(folder === '/' ? '' : folder)}
                                                    style={{
                                                        padding: '10px 14px',
                                                        paddingLeft: `${14 + (depth * 20)}px`, // ایجاد تو‌رفتگی منظم بر اساس عمق پوشه
                                                        borderRadius: '6px',
                                                        cursor: 'pointer',
                                                        background: isSelected ? 'rgba(168, 85, 247, 0.15)' : 'transparent',
                                                        border: isSelected ? '1px solid var(--ch-accent)' : '1px solid transparent',
                                                        color: isSelected ? 'white' : 'var(--ch-text-dim)',
                                                        display: 'flex',
                                                        alignItems: 'center',
                                                        gap: '10px',
                                                ...
                                                >
                                                    {/* رندر خطوط راهنمای عمودی کم‌رنگ برای اتصال بصری زیرشاخه‌ها */}
                                                    {!isRoot && depth > 1 && (
                                                        <div style={{
                                                            position: 'absolute',
                                                            left: `${14 + ((depth - 1) * 20) + 6}px`,
                                                            top: '-10px',
                                                            bottom: '50%',
                                                            width: '1px',
                                                    ...
                                                    <Folder 
                                                        size={16} 
                                                        style={{ 
                                                            color: isSelected ? 'var(--ch-accent)' : '#3b82f6', 
                                                            flexShrink: 0,
