...
const SearchResults = () => {
    ...
    useEffect(() => {
        const fetchSearchResults = async () => {
            ...
            // ۱. بررسی سشن‌استوریج: کش کردن امن بدون ایجاد تداخل در حافظه ری‌اکت
            const cacheKey = `search_cache_${searchQuery}`;
            ...
            if (cachedData) {
                try {
                    ...
                    setIsLoading(false);
                    return; // خروج از تابع برای جلوگیری از درخواست به سرور
                } catch (e) {
                    console.error("خطا در خواندن کش:", e);
                }
            }
            ...
            try {
                ...
                if (data.is_success) {
                    const fetchedResults = data.response || [];
                    setResults(fetchedResults);
                    
                    // ۲. ذخیره نتایج در سشن‌استوریج برای بازگشت‌های بعدی
                    sessionStorage.setItem(cacheKey, JSON.stringify(fetchedResults));
                } else {
